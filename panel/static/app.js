...
const gb = (bytes) => bytes == null ? 'Unavailable' : (bytes / 1073741824).toFixed(1) + ' GB';
...
async function action(name, data) {
  const response = await fetch('/api/actions/' + name, {method: 'POST', headers: {'Content-Type': 'application/json', 'X-CSRF-Token': csrf}, body: JSON.stringify(data)});
...
function render() {
  ...
  $('#service-list').innerHTML = Object.entries(metrics.services).map(([name, up]) => `<div class="service"><strong>${escapeHTML(name)}</strong><span class="status ${up ? '' : 'failed'}">${up ? 'Responding' : 'Unavailable'}</span></div>`).join('') + `<div class="service"><strong>Container memory</strong><span>${gb(metrics.memory_bytes)} / ${metrics.memory_limit == null ? 'No cgroup limit' : gb(metrics.memory_limit)}</span></div>`;
  $('#metric-scope').textContent = metrics.scope;
  $('#backup-list').innerHTML = overview.backups.length ? `<table><thead><tr><th>Website</th><th>Created</th><th>Recovery</th></tr></thead><tbody>${overview.backups.map(item => `<tr><td>${escapeHTML(item.hostname)}</td><td>${date(item.created)}</td><td><button data-restore="${item.id}">Restore</button></td></tr>`).join('')}</tbody></table>` : '<div class="empty">No recovery points yet.</div>';
  $('#job-list').innerHTML = overview.jobs.length ? overview.jobs.map(job => `<div class="job"><div><p><strong>${escapeHTML(job.action)}</strong> &middot; ${escapeHTML(job.message)}</p><small>${date(job.created)}</small></div><span class="status ${escapeHTML(job.status)}">${escapeHTML(job.status)}</span></div>`).join('') : '<div class="empty">No recent operations.</div>';
...
async function refresh() {
  try {
    ...
    if (!response.ok) throw new Error('Measurements unavailable. Retrying shortly.');
    overview = await response.json(); render();
  } catch (error) { notice(error.message, true); $('#updated').textContent = 'Connection unavailable'; }
}
...
$('#site-form select').addEventListener('change', (event) => {
  ...
  $('#cms-fields').querySelectorAll('input').forEach(input => { input.required = !$('#cms-fields').hidden; });
});
...
handleForm('#settings-form', async form => {
  ...
  await action('backup.schedule', {site: data.site, enabled: form.elements.enabled.checked, retention: Number(data.retention)});
});
handleForm('#restore-form', form => action('backup.restore', Object.fromEntries(new FormData(form))));
$('#search').addEventListener('input', renderSites);
document.addEventListener('click', async event => {
  const button = event.target.closest('button'); if (!button) return;
  if (button.dataset.settings) {
    const site = overview.sites.find(s => s.id === button.dataset.settings); const form = $('#settings-form');
    $('#settings-host').textContent = site.hostname; form.elements.site.value = site.id;
    ['memory', 'upload', 'timeout'].forEach(key => { form.elements[key].value = site.php[key]; });
    form.elements.enabled.checked = site.daily_backup; form.elements.retention.value = site.retention; $('#settings-dialog').showModal();
  }
  if (button.dataset.backup) { button.disabled = true; try { await action('backup.create', {site: button.dataset.backup}); } catch(error) { notice(error.message, true); } finally { button.disabled = false; } }
  if (button.dataset.restore) {
    const item = overview.backups.find(b => b.id === button.dataset.restore); const form = $('#restore-form');
    form.reset(); form.elements.site.value = item.site; form.elements.backup.value = item.id; $('#restore-target').textContent = item.hostname + ' / ' + date(item.created); $('#restore-dialog').showModal();
  }
});
refresh();
